...
function addMessage (shell, eventObject) {
    ...
    bubble.header.contents[0].text = g.mapping_table.philio.function_type[funcType]  ;
    ...
    switch (eventCode) {
        ...
        case 5002 :  {      // Meter switch  
            if (funcType == 22) 
                bubble.body.contents[0].text = funcType + " is " +  ( eventObject.basicValue  == 0?"Off":"On") ;
            else 
    ...
    bubble.footer.contents[0].text = d ; 
    switch (battery){
